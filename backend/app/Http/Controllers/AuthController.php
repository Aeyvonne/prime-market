...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        try {
            $data = $request->validate([
                ...
                // Champs Admin Sectoriel
                'secteur'            => $request->input('role') === 'admin_sectoriel' ? 'required|in:Agriculture,Elevage,Peche' : 'sometimes|nullable',
                'zone_intervention'  => 'sometimes|nullable|string|max:255',
                'disponibilite'      => 'sometimes|nullable',
            ]);

            // Les producteurs nécessitent validation admin, les autres rôles sont actifs
            $statut = $data['role'] === 'producteur' ? 'en_attente' : 'actif';
            ...
            return DB::transaction(function () use ($data, $statut) {
                ...
                if ($user->role === 'producteur') {
                    $message = 'Compte créé avec succès. Veuillez uploader vos documents justificatifs pour validation par un administrateur.';
    ...
    public function login(Request $request)
    {
        ...
        if ($user->statut === 'en_attente') {
            $message = 'Votre compte est en attente de validation par un administrateur. Veuillez uploader vos documents justificatifs.';
            $pendingApproval = true;
        }
    ...
    public function profil(Request $request)
    {
        ...
        elseif ($user->role === 'admin_sectoriel') $user->load('adminSectorielle');
        ...
        if ($user->role === 'admin_sectoriel' && $user->adminSectorielle) $userData['admin_sectorielle'] = $user->adminSectorielle;
    ...
    private function formatUser(User $user): array
    {
        ...
        if ($user->relationLoaded('adminSectorielle') && $user->adminSectorielle) {
            $data['admin_sectorielle'] = $user->adminSectorielle;
        } elseif ($user->role === 'admin_sectoriel') {
            $user->load('adminSectorielle');
            $data['admin_sectorielle'] = $user->adminSectorielle;
        }
