  ...
  <div class="space-y-8 animate-in fade-in duration-700">
    <div class="bg-gradient-to-br from-[#2D5A27] to-[#1E3F1A] rounded-[2.5rem] p-8 md:p-12 text-white relative overflow-hidden shadow-xl shadow-[#2D5A27]/10">
      ...
      <div class="relative z-10">
        <h1 class="text-3xl md:text-4xl font-black font-serif leading-tight mb-2">Validations Producteurs</h1>
        <p class="text-white/80 text-sm md:text-base font-medium">Vérifiez les documents des producteurs en attente d'activation.</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div v-else-if="comptes.length === 0" class="text-center py-16">
        ...
        <p class="text-sm text-gray-400 font-medium mt-1">Tous les producteurs ont été vérifiés.</p>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50/50">
              ...
              <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Secteur</th>
              <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Documents</th>
              <th class="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Inscrit le</th>
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="c in comptes" :key="c.id" class="hover:bg-[#F5F0E8]/20 transition-all">
              ...
              <td class="px-8 py-5">
                <span class="px-3 py-1 bg-[#F5F0E8] text-[#8B7340] rounded-full text-[10px] font-bold uppercase">{{ c.secteur_travail }}</span>
              </td>
              ...
              <td class="px-8 py-5 text-center text-xs text-gray-500 font-medium">{{ formatDate(c.created_at) }}</td>
              <td class="px-8 py-5 text-right">
                <NuxtLink :to="`/admin/super/comptes/${c.id}/documents`"
...
async function load() {
  ...
  try {
    comptes.value = await $fetch(`${apiUrl}/super-admin/comptes/en-attente`, {
