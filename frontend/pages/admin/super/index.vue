...
onMounted(async () => {
  ...
  try {
    const statsRes = await fetch('http://127.0.0.1:8000/api/super-admin/statistiques', { headers })
    if (!statsRes.ok) throw new Error(`Stats HTTP ${statsRes.status}`)
    rawStats.value = await statsRes.json()
  } catch (err) {
    console.error('Erreur chargement statistiques:', err)
  }

  try {
    const txRes = await fetch('http://127.0.0.1:8000/api/super-admin/transactions', { headers })
    if (!txRes.ok) throw new Error(`Transactions HTTP ${txRes.status}`)
    const data = await txRes.json()
    recentTransactions.value = Array.isArray(data) ? data.slice(0, 5) : []
  } catch (err) {
