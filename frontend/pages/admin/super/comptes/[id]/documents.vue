<template>
  <div class="space-y-8 animate-in fade-in duration-700">
    <div class="flex items-center justify-between">
      <div>
        <NuxtLink to="/admin/super/validations" class="text-sm font-bold text-[#8B7340] hover:underline mb-2 inline-block">← Retour aux validations</NuxtLink>
...
async function activateAccount() {
  ...
  try {
    await $fetch(`${apiUrl}/super-admin/comptes/${producteur.value.id}`, {
      method: 'PUT', headers: { Authorization: `Bearer ${token}` }, body: { statut: 'actif' }
    })
...
async function load() {
  const token = localStorage.getItem('token')
  try {
    const res = await $fetch(`${apiUrl}/super-admin/comptes/${route.params.id}/documents`, {
...
async function verifyDoc(id) {
  const token = localStorage.getItem('token')
  try {
    const res = await $fetch(`${apiUrl}/super-admin/documents/${id}/verifier`, {
...
async function rejectDoc() {
  ...
  const token = localStorage.getItem('token')
  try {
    const res = await $fetch(`${apiUrl}/super-admin/documents/${rejectModal.doc.id}/rejeter`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token}` },
