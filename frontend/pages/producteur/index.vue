<template>
  <div class="space-y-8 animate-in fade-in duration-700">
    ...
    <div v-if="isPending" class="bg-amber-50 border border-amber-200 rounded-[2rem] p-6 flex items-start gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-sm text-amber-700 mt-1">Veuillez fournir vos documents justificatifs dans la section <NuxtLink to="/producteur/documents" class="font-bold underline">Mes documents</NuxtLink> pour permettre à un administrateur de valider votre compte.</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div v-if="!isPending" class="lg:col-span-2 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8 border-b border-gray-50 flex items-center justify-between">
          <h3 class="text-xl font-black text-gray-900">Mes Derniers Produits</h3>
          <NuxtLink v-if="!isPending" to="/producteur/produits" class="text-xs font-bold text-[#2D5A27] hover:underline">Voir tout →</NuxtLink>
        </div>
      ...
      <div v-if="!isPending" class="bg-[#2D5A27] rounded-[2.5rem] shadow-xl p-8 text-white relative overflow-hidden flex flex-col justify-between">
...
const isPending = computed(() => {
  try { return JSON.parse(localStorage.getItem('user') || '{}').statut === 'en_attente' }
  catch { return false }
})
...
async function chargerDashboardData() {
  ...
  try {
    ...
    const [produitsData, commandesData, ventesData] = await Promise.all([
      $fetch('http://127.0.0.1:8000/api/producteur/produits', { headers }),
      $fetch('http://127.0.0.1:8000/api/producteur/commandes', { headers }),
      $fetch('http://127.0.0.1:8000/api/producteur/ventes', { headers }),
    ])

    produits.value = produitsData || []
    const commandes = commandesData || []
    const enCours = commandes.filter(c => c.status === 'EnCours').length
    const ventes = ventesData?.commandes || []
    const revenuMois = Number(ventes
      .filter(v => {
        const d = new Date(v.date_commande)
        const now = new Date()
        return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()
      })
      .reduce((sum, v) => sum + Number(v.montant_total || 0), 0))

    stats.value[0].value = enCours.toString()
    stats.value[1].value = produits.value.length.toString()
    stats.value[2].value = formatPrice(revenuMois)
