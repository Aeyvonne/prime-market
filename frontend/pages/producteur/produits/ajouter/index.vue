<template>
  <div class="max-w-4xl mx-auto animate-in slide-in-from-bottom duration-700">
    ...
    <div class="mb-8">
      ...
      <h1 class="text-3xl font-black text-gray-900 font-serif">Ajouter un <span class="text-[#2D5A27]">Produit</span></h1>
      <p class="text-gray-500 font-medium">Ajoutez un nouveau produit à votre catalogue de vente.</p>
    </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div class="space-y-2">
            ...
            <select v-model="selectedCategory" required class="w-full px-6 py-4 bg-[#F5F0E8]/40 border-2 border-transparent rounded-2xl focus:border-[#2D5A27] focus:bg-white outline-none transition-all font-bold text-gray-700">
          ...
          <div class="space-y-2">
            ...
            <select v-model="form.sous_categorie_id" :disabled="!selectedCategory" required class="w-full px-6 py-4 bg-[#F5F0E8]/40 border-2 border-transparent rounded-2xl focus:border-[#2D5A27] focus:bg-white outline-none transition-all font-bold text-gray-700 disabled:opacity-50">
...
async function handleSubmit() {
  ...
  try {
    ...
    await $fetch('http://127.0.0.1:8000/api/producteur/produits', {
      method: 'POST',
      headers: {
    ...
    showPopup('success', 'Félicitations !', 'Produit mis en vente avec succès.', '/producteur/produits')
  } catch (err) {
