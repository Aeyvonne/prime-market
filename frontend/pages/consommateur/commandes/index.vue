    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      <div class="flex items-center gap-2 p-1 bg-white rounded-2xl shadow-sm border border-gray-100">
        <button v-for="s in statusFilters" :key="s.value" @click="selectedStatus = s.value"
          ...
          :class="[selectedStatus === s.value ? 'bg-[#2D5A27] text-white shadow-lg shadow-[#2D5A27]/20' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50']">{{ s.label }}</button>
      </div>
    </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50/50">
              ...
              <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Produits</th>
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="order in filteredOrders" :key="order.id" class="hover:bg-[#F5F0E8]/30 transition-colors group">
              <td class="px-8 py-6"><span class="text-sm font-bold text-gray-900">#{{ order.id }}</span></td>
              <td class="px-8 py-6">
              ...
              <td class="px-8 py-6"><span class="text-sm text-gray-500 font-medium">{{ formatDate(order.date_commande) }}</span></td>
              <td class="px-8 py-6"><span class="text-sm font-bold text-gray-900">{{ order.lignes_commande?.length || 0 }}</span></td>
              <td class="px-8 py-6"><span class="text-sm font-black text-gray-900">{{ formatPrice(order.montant_total) }}</span></td>
              ...
              <td class="px-8 py-6 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button v-if="order.status === 'EnCours'" @click="handleCancel(order.id)"
                    class="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-[10px] font-black hover:bg-red-100 transition-all">Annuler</button>
                  <button v-if="order.status === 'Valider' && order.livraison?.status === 'Livrée'" @click="handleConfirm(order.id)"
                    class="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-black hover:bg-emerald-100 transition-all">Confirmer</button>
                  <NuxtLink :to="`/consommateur/commandes/${order.id}`"
                    class="p-2.5 bg-gray-100 text-gray-600 hover:bg-[#2D5A27] hover:text-white rounded-xl transition-all inline-flex items-center justify-center">
            ...
            <tr v-if="!pending && filteredOrders.length === 0">
              <td colspan="7" class="px-8 py-20 text-center text-gray-400 font-medium italic">Aucune commande trouvée.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const { data: orders, pending, refresh } = await useFetch('http://127.0.0.1:8000/api/consommateur/commandes', {
...
async function handleCancel(id) {
  ...
  try {
    await $fetch(`http://127.0.0.1:8000/api/consommateur/commandes/${id}/annuler`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
    })
    refresh()
...
async function handleConfirm(id) {
  ...
  try {
    await $fetch(`http://127.0.0.1:8000/api/consommateur/commandes/${id}/confirmer-reception`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
    })
    refresh()
