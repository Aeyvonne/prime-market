      ...
      <div v-else-if="filteredProducts.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        >
          ...
          <div class="p-6 flex flex-col flex-grow">
            ...
            <!-- CTA -->
            <NuxtLink :to="`/catalogue/${product.id}`" class="mt-6 w-full py-3.5 rounded-xl border-2 border-[#2D5A27] text-[#2D5A27] font-bold text-sm transition-all duration-300 flex items-center justify-center gap-2 group-hover:bg-[#2D5A27] group-hover:text-white">
              Voir détails
              <svg class="w-4 h-4 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
            </NuxtLink>
          </div>
        </div>
      </div>
