...
onMounted(async () => {
  if (!productId) return
  try {
    const product = await $fetch(`http://127.0.0.1:8000/api/produits/${productId}`)
    if (product) {
...
async function handleSubmit() {
  ...
  try {
    ...
    await $fetch(`http://127.0.0.1:8000/api/distributeur/produits/${productId}`, {
    ...
    showPopup('success', 'Mise à jour réussie !', 'Le produit a été modifié avec succès.', '/distributeur/produits')
  } catch (err) {
