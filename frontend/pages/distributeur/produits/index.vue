  ...
  <div class="space-y-8 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        ...
        <p class="text-gray-500 font-medium mt-1">Gérez votre inventaire destiné à la revente.</p>
      </div>
      
      <NuxtLink to="/distributeur/produits/nouveau" class="px-8 py-4 bg-[#2D5A27] text-white rounded-2xl font-black text-sm shadow-xl shadow-[#2D5A27]/20 hover:-translate-y-1 transition-all active:scale-95 flex items-center gap-3">
            ...
            <template v-if="products && products.length > 0">
              ...
              <td class="px-8 py-6">
                <div class="flex items-center gap-4">
                  <div class="w-12 h-12 rounded-xl bg-[#F5F0E8] flex items-center justify-center text-2xl group-hover:scale-110 transition-transform overflow-hidden">
                    <img v-if="product.photo" :src="`http://127.0.0.1:8000/storage/products/${product.photo}`" :alt="product.nom" class="w-full h-full object-cover" />
...
async function fetchProducts() {
  ...
  try {
    ...
    const response = await $fetch('http://127.0.0.1:8000/api/distributeur/produits', {
      headers: {
...
function editProduct(id) {
  navigateTo(`/distributeur/produits/${id}/modifier`)
}
...
async function executeDelete() {
  ...
  try {
    ...
    await $fetch(`http://127.0.0.1:8000/api/distributeur/produits/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: token ? `Bearer ${token}` : ''
      }
