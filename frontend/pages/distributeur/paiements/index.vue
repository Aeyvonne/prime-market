        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="paiement in paiements" :key="paiement.id" class="hover:bg-[#F5F0E8]/30 transition-colors group">
              <td class="px-8 py-6">
                <span class="text-sm font-bold text-gray-900 uppercase">#PAY-{{ paiement.id }}</span>
              </td>
              <td class="px-8 py-6">
                <NuxtLink :to="`/distributeur/commandes/${paiement.commande_id}`" class="text-sm font-bold text-[#2D5A27] hover:underline">
                  Cmd #{{ paiement.commande_id }}
                </NuxtLink>
              </td>
              <td class="px-8 py-6">
                <span class="text-sm text-gray-500 font-medium">{{ formatDate(paiement.date_transaction) }}</span>
              </td>
              <td class="px-8 py-6">
                <div class="flex items-center gap-2">
                  <span class="text-xs font-bold text-gray-700 bg-gray-100 px-3 py-1 rounded-lg">
                    {{ paiement.mode_paiement || 'Mobile Money' }}
                  </span>
                </div>
              </td>
              <td class="px-8 py-6">
                <span class="text-sm font-black text-gray-900">{{ formatPrice(paiement.montant) }}</span>
              </td>
              <td class="px-8 py-6 text-center">
                <span :class="`inline-flex px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider text-center ${paiement.statut === 'Payé' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`">
                  {{ paiement.statut }}
                </span>
              </td>
            </tr>
...
const { data: paiements, pending } = await useFetch('http://127.0.0.1:8000/api/distributeur/paiements', {
  headers: {
...
const totalMonth = computed(() => {
  if (!paiements.value) return 0
  const currentMonth = new Date().getMonth()
  return paiements.value
    .filter(p => p.statut === 'Payé' && new Date(p.date_transaction).getMonth() === currentMonth)
