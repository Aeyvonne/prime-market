    ...
    <div v-else-if="livraison" class="max-w-4xl mx-auto space-y-8">
      ...
      <div class="bg-white p-8 md:p-12 rounded-[3rem] shadow-sm border border-gray-100">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          ...
          <div :class="`px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest ${livraison.status === 'Valider' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700 animate-pulse'}`">
        ...
        <div class="relative mb-16 px-4">
          <div class="absolute top-5 left-8 right-8 h-1 bg-gray-100 rounded-full">
            <div :class="`h-full bg-[#2D5A27] transition-all duration-1000 ${livraison.status === 'Valider' ? 'w-full' : 'w-1/2'}`"></div>
          </div>
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p class="text-sm font-bold text-gray-700">{{ formatDate(livraison.date_livraison_estimee) }}</p>
                </div>
              </div>
...
const steps = [
  { label: 'Préparée', status: 'preparation', icon: '📦' },
  { label: 'En Transit', status: 'transit', icon: '🚚' },
  { label: 'Livrée', status: 'livree', icon: '🏁' }
]
...
function isStepDone(stepStatus) {
  ...
  if (stepStatus === 'preparation') return true
  if (stepStatus === 'transit') return livraison.value.status !== 'EnCours'
  if (stepStatus === 'livree') return livraison.value.status === 'Livrer'
  return false
}
