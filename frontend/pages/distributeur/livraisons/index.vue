    ...
    <div class="grid grid-cols-1 gap-6">
      ...
      >
        <div class="flex items-center gap-6">
          <div :class="`w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-2xl shadow-inner ${liv.status === 'Valider' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600 animate-pulse'}`">
            {{ liv.status === 'Valider' ? '✅' : '🚚' }}
          </div>
          <div>
            <div class="flex items-center gap-3 mb-1">
              <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Livraison #{{ liv.id }}</span>
              <span :class="`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider ${liv.status === 'Valider' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`">
                {{ statusLabel(liv.status) }}
              </span>
            </div>
        ...
        <div class="flex-grow max-w-md hidden lg:block">
          ...
          <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
            <div :class="`h-full transition-all duration-1000 ${liv.status === 'Valider' ? 'w-full bg-emerald-500' : 'w-1/2 bg-amber-500'}`"></div>
          </div>
        </div>
