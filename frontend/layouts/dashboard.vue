    ...
    <main class="flex-grow lg:ml-72 flex flex-col min-h-screen">
      ...
      <header class="h-20 bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40 px-4 sm:px-8 flex items-center justify-between shadow-sm">
        ...
        <div class="flex items-center gap-3 sm:gap-6">
          <ClientOnly>
            <!-- Panier -->
            <NuxtLink v-if="userRole === 'consommateur' || userRole === 'distributeur'"
              to="/consommateur/panier"
              class="relative p-2.5 text-gray-500 hover:bg-gray-100 rounded-2xl transition-colors group">
              <svg class="w-6 h-6 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z"/></svg>
              <span v-if="cartCount > 0"
                class="absolute -top-0.5 -right-0.5 w-5 h-5 bg-red-500 border-2 border-white rounded-full flex items-center justify-center text-[9px] font-black text-white">
...
<script setup>
definePageMeta({
  middleware: ['auth', 'role', 'pending-producer'],
})
...
const currentPageTitle = computed(() => {
  ...
  const titles = {
    ...
    validations: 'Validations Producteurs'
  }
...
const navByRole = {
  ...
  consommateur: [
    ...
    { label: 'Évaluations', to: '/consommateur/evaluations', icon: IconStats },
    { label: 'Mon profil', to: '/profil', icon: IconProfile },
  ],
  transporteur: [
    { label: 'Tableau de bord', to: '/transporteur', icon: IconDashboard },
    { label: 'Missions', to: '/transporteur/missions', icon: IconTruck },
  ...
  super_administrateur: [
    ...
    { label: 'Validations producteurs', to: '/admin/super/validations', icon: IconStats },
  ...
  admin_sectoriel: [
    { label: 'Tableau de bord', to: '/admin/sectoriel', icon: IconDashboard },
    { label: 'Comptes Producteurs', to: '/admin/sectoriel/comptes', icon: IconProfile },
    { label: 'Validations documents', to: '/admin/sectoriel/validations', icon: IconStats },
    { label: 'Sous-Catégories', to: '/admin/sectoriel/sous-categories', icon: IconProducts },
    { label: 'Transactions', to: '/admin/sectoriel/transactions', icon: IconWallet },
    { label: 'Mon profil', to: '/profil', icon: IconProfile },
  ]
}

const userStatut = ref('')

const isPendingProducer = computed(() => {
  return userRole.value === 'producteur' && userStatut.value === 'en_attente'
})

const currentNav = computed(() => {
  const nav = navByRole[userRole.value] || navByRole.consommateur
  if (isPendingProducer.value) {
    return nav.filter(item =>
      item.to === '/producteur' ||
      item.to === '/producteur/documents' ||
      item.to === '/profil'
    )
  }
  return nav.map(item => {
    if ((item.to === '/admin/super/validations' || item.to === '/admin/sectoriel/validations') && pendingValidationCount.value > 0) {
      return { ...item, badge: pendingValidationCount.value }
    }
    if (item.to === '/producteur/commandes' && commandesBadgeCount.value > 0) {
      return { ...item, badge: commandesBadgeCount.value }
    }
    if (item.to === '/distributeur/commandes' && commandesBadgeCount.value > 0) {
      return { ...item, badge: commandesBadgeCount.value }
    }
    if (item.to === '/consommateur/commandes' && commandesBadgeCount.value > 0) {
      return { ...item, badge: commandesBadgeCount.value }
    }
    if (item.to === '/transporteur/missions' && missionsBadgeCount.value > 0) {
      return { ...item, badge: missionsBadgeCount.value }
    }
    return item
  })
})
...
async function fetchBadges() {
  ...
  if (!token) return
  try {
    const res = await $fetch('http://127.0.0.1:8000/api/badges', {
      headers: { Authorization: `Bearer ${token}` }
    })
    commandesBadgeCount.value = res.commandes || 0
    missionsBadgeCount.value = res.missions || 0
  } catch {
    commandesBadgeCount.value = 0
    missionsBadgeCount.value = 0
  }
}

async function fetchPendingCount() {
  if (userRole.value !== 'super_administrateur' && userRole.value !== 'admin_sectoriel') return
  const token = localStorage.getItem('token')
  const endpoint = userRole.value === 'super_administrateur'
    ? '/api/super-admin/comptes/en-attente/count'
    : '/api/admin-sectoriel/comptes/en-attente/count'
  try {
    const res = await $fetch(`http://127.0.0.1:8000${endpoint}`, {
      headers: { Authorization: token ? `Bearer ${token}` : '' }
    })
    pendingValidationCount.value = res.count || 0
  } catch { pendingValidationCount.value = 0 }
}
...
async function refreshUserProfile() {
  ...
  if (!token) return
  try {
    const res = await fetch('http://127.0.0.1:8000/api/profil', {
      headers: { Authorization: `Bearer ${token}` }
    })
    if (res.ok) {
